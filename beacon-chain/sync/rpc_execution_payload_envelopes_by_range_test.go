...
func TestSendExecutionPayloadEnvelopesByRangeRequest(t *testing.T) {
	...
	t.Run("receives envelopes from remote peer", func(t *testing.T) {
		...
		p2.SetStreamHandler(topic, func(stream network.Stream) {
			...
			for i := range count {
				...
				assert.NoError(t, WriteExecutionPayloadEnvelopeChunk(stream, clock, p2.Encoding(), env))
			}
		})
	...
	t.Run("slots not monotonically increasing returns error", func(t *testing.T) {
		...
		p2.SetStreamHandler(topic, func(stream network.Stream) {
			...
			assert.NoError(t, WriteExecutionPayloadEnvelopeChunk(stream, clock, p2.Encoding(), env1))
			assert.NoError(t, WriteExecutionPayloadEnvelopeChunk(stream, clock, p2.Encoding(), env2))
		})
